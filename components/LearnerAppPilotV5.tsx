...
import { readSubmissions, saveSubmission, type AiReviewAnswer, type JourneySubmission } from "@/lib/storage";
import { Card, Pill, PrimaryButton, TextArea, TextInput } from "@/components/ui";

type FirstChoice = "A" | "B";
type SecondChoice = "기존 유지" | "일부 수정" | "판단 변경";
type ReviewChoice = "O" | "X" | "모르겠음";
type Tone = "blue" | "teal" | "purple" | "orange" | "green" | "gray";

const PILOT_SESSION_CODE = "PILOT-CHA-BIO-01";
...
const secondOptions: { value: SecondChoice; title: string; desc: string; tone: "blue" | "teal" | "purple"; icon: string }[] = [
  { value: "기존 유지", title: "처음 판단의 방향을 유지", desc: "돌발상황을 보아도 1차 판단의 큰 방향은 그대로 둡니다.", tone: "blue", icon: "🛡️" },
  { value: "일부 수정", title: "방향은 유지하되 조건을 조정", desc: "핵심 방향은 살리되 범위·순서·설명 방식을 수정합니다.", tone: "teal", icon: "🔧" },
  { value: "판단 변경", title: "처음 판단과 다른 방향 선택", desc: "새로운 변수가 결정적으로 커져 판단 방향을 바꿉니다.", tone: "purple", icon: "↗️" },
];
...
function normalizeSecond(value: string | undefined): SecondChoice | null {
  if (value === "기존 유지" || value === "유지") return "기존 유지";
  if (value === "일부 수정" || value === "보완") return "일부 수정";
  if (value === "판단 변경" || value === "전환") return "판단 변경";
  return null;
}

function normalizeReview(value: string | undefined): ReviewChoice | "" {
  if (value === "O" || value === "Yes") return "O";
  if (value === "X" || value === "No") return "X";
  if (value === "모르겠음") return "모르겠음";
  return "";
}
...
function BrandHeader() {
  return (
    <div className="mb-4 flex items-center gap-3 rounded-2xl border border-white/70 bg-white/90 px-3 py-2 shadow-sm backdrop-blur-md">
...
function Shell({ children, step, scrollRef }: { children: React.ReactNode; step: number; scrollRef: React.RefObject<HTMLDivElement> }) {
  const labels = ["홈", "상황", "1차", "돌발", "2차", "AI", "결정"];
  return (
    <div className="mx-auto flex min-h-[860px] w-full max-w-[430px] flex-col overflow-hidden rounded-[2.4rem] border border-slate-200 bg-slate-50 shadow-2xl">
      <div className="bg-white/95 px-5 pb-2 pt-5 backdrop-blur">
        <BrandHeader />
        <div className="flex justify-center gap-2">
          {labels.map((_, i) => <span key={i} className={`h-2.5 w-2.5 rounded-full ${i <= step ? "bg-blue-600" : "bg-slate-200"}`} />)}
        </div>
      </div>
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-5 py-5">{children}</div>
      <div className="grid grid-cols-5 gap-1 border-t border-slate-100 bg-white px-3 py-3 text-center text-[10px] text-slate-400">
        {["홈", "라운드", "로그북", "코칭", "내 정보"].map((s, i) => <div key={s} className={i === 0 ? "font-bold text-blue-600" : ""}>●<br />{s}</div>)}
      </div>
    </div>
  );
}
...
function Info({ title, children, tone = "blue" }: { title: string; children: React.ReactNode; tone?: Tone }) { return <Card className="p-4"><span className={`mb-3 inline-flex rounded-full px-3 py-1 text-xs font-extrabold ${toneMap[tone]}`}>{title}</span><div className="text-sm leading-7 text-slate-600">{children}</div></Card>; }
function ImageHero({ compact = false }: { compact?: boolean }) { return <div className={`relative mt-5 overflow-hidden rounded-[2rem] shadow-inner ${compact ? "h-44" : "h-52"}`}><div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: "url('/lighthouse-hero.png')" }} /><div className="absolute inset-0 bg-gradient-to-r from-white/88 via-white/45 to-blue-50/10" /><div className="absolute inset-x-0 bottom-0 h-20 bg-gradient-to-t from-white/85 to-transparent" /><div className="absolute left-5 top-5 rounded-full bg-white/85 px-3 py-1 text-xs font-bold text-blue-700 shadow-sm">판단 여정</div><div className="absolute left-5 top-14 max-w-[190px] rounded-2xl bg-white/80 px-4 py-3 text-sm font-extrabold leading-6 text-slate-700 shadow-sm">선택의 길을 따라<br />판단 기준을 세웁니다</div></div>; }
function ProgressRing({ completed }: { completed: number }) { const percent = Math.round((completed / 4) * 100); return <div className="flex h-28 w-28 items-center justify-center rounded-full bg-gradient-to-br from-blue-600 via-blue-500 to-violet-500 p-2 shadow-lg shadow-blue-100"><div className="flex h-full w-full flex-col items-center justify-center rounded-full bg-white"><b className="text-2xl text-slate-900">{completed}/4</b><span className="text-[11px] font-bold text-slate-500">완료 라운드</span><span className="mt-1 text-[10px] text-slate-400">{percent}%</span></div></div>; }
function RoundMiniCard({ title, index, active, done, onClick }: { title: string; index: number; active?: boolean; done?: boolean; onClick: () => void }) { const icon = ["📋", "👥", "💻", "☕"][index % 4]; return <button onClick={onClick} className={`min-h-[158px] rounded-3xl border p-4 text-left shadow-sm transition ${active ? "border-blue-400 bg-blue-50 ring-4 ring-blue-100" : "border-slate-100 bg-white"}`}><div className="flex items-start justify-between"><Pill tone={done ? "green" : active ? "blue" : "gray"}>라운드 {index + 1}</Pill><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-slate-50 text-2xl shadow-inner">{icon}</span></div><p className="mt-4 min-h-[42px] text-sm font-black leading-6 text-slate-800">{title}</p><div className="mt-3 flex items-center gap-1">{[0, 1, 2, 3, 4].map((i) => <span key={i} className={`h-2.5 w-2.5 rounded-full ${done ? "bg-blue-600" : "bg-slate-200"}`} />)}</div><p className="mt-3 text-xs font-bold text-slate-500">{done ? "완료" : active ? "선택됨" : "대기"}</p></button>; }
function StepHero({ badge, title, subtitle, icon, tone = "blue" }: { badge: string; title: string; subtitle: string; icon: string; tone?: Tone }) { return <div className="rounded-[2rem] bg-gradient-to-br from-white via-blue-50/60 to-teal-50 p-5 shadow-sm"><div className="flex items-center justify-between"><Pill tone={tone}>{badge}</Pill><span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white text-3xl shadow-inner">{icon}</span></div><h2 className="mt-4 text-2xl font-black leading-tight text-slate-900">{title}</h2><p className="mt-2 text-sm leading-6 text-slate-500">{subtitle}</p></div>; }
function FirstOptionCard({ label, title, selected, onClick, tone }: { label: FirstChoice; title: string; selected: boolean; onClick: () => void; tone: "blue" | "teal" }) { const selectedClass = tone === "blue" ? "border-blue-400 bg-blue-50 ring-blue-100" : "border-teal-400 bg-teal-50 ring-teal-100"; const badge = tone === "blue" ? "bg-blue-600" : "bg-teal-600"; return <button onClick={onClick} className={`w-full rounded-3xl border p-4 text-left transition ${selected ? `${selectedClass} ring-4` : "border-slate-200 bg-white"}`}><div className="flex gap-3"><div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl text-lg font-black text-white ${badge}`}>{label}</div><div className="flex-1"><div className="text-sm font-bold text-slate-400">1차 선택안</div><p className="mt-1 text-base font-extrabold leading-6 text-slate-800">{title}</p></div><div className={`mt-1 h-5 w-5 rounded-full border-2 ${selected ? "border-blue-600 bg-blue-600" : "border-slate-300"}`} /></div></button>; }
...
export default function LearnerAppPilotV5() {
  ...
  const [completedRoundIds, setCompletedRoundIds] = useState<number[]>([]);
  ...
  const [reviewAnswers, setReviewAnswers] = useState<(ReviewChoice | "")[]>(reviewItems.map(() => ""));
  const [reviewNote, setReviewNote] = useState("");
  const [finalLines, setFinalLines] = useState(["", "", "", "", ""]);
  const round = rounds[roundIndex];
  const filled = (v: string) => v.trim().length > 0;
  const participantName = team.trim() || "참여자";
  ...
  const needReviewNote = reviewAnswers.some((a) => a === "X" || a === "모르겠음");
  const canReview = reviewAnswers.every(Boolean) && (!needReviewNote || filled(reviewNote));
  const canFinal = finalLines.every(filled);
  const finalPreview = finalLines.filter(filled).join("\n");
  const completedCount = completedRoundIds.filter((id) => id >= 1 && id <= 4).length;
  const refreshCompleted = (teamName: string) => { const ids = readSubmissions().filter((s) => s.sessionCode === PILOT_SESSION_CODE && s.teamName === teamName).map((s) => s.roundId); setCompletedRoundIds(Array.from(new Set(ids))); };
  ...
  const loadSavedRound = (teamName: string, roundId: number) => { const saved = readSubmissions().filter((s) => s.sessionCode === PILOT_SESSION_CODE && s.teamName === teamName && s.roundId === roundId).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0]; if (!saved) { reset(); return; } setFirstChoice(saved.firstChoice); setFirstReason(saved.firstReason || ""); setConcern(saved.concern || ""); setSecondChoice(normalizeSecond(saved.secondChoice)); setSecondAnswers([0,1,2].map((i) => saved.secondQuestionAnswers?.[i]?.answer || "")); setAiSummary(saved.aiAnswerSummary || saved.aiFeedbackSummary || ""); setReviewAnswers(reviewItems.map((q) => normalizeReview(saved.aiReviewResults?.find((r) => r.question === q)?.answer))); setReviewNote(saved.aiReviewNote || ""); setFinalLines([0,1,2,3,4].map((i) => saved.finalLines?.[i] || "")); };
  useEffect(() => { const savedTeam = window.localStorage.getItem(TEAM_STORAGE_KEY); if (savedTeam) { setTeam(savedTeam); refreshCompleted(savedTeam); setScreen(1); } }, []);
  useEffect(() => { const id = window.setTimeout(() => { scrollRef.current?.scrollTo({ top: 0, behavior: "auto" }); window.scrollTo({ top: 0, behavior: "auto" }); }, 0); return () => window.clearTimeout(id); }, [screen]);
  const prompt = useMemo(() => `나는 팀장 역할로 업무관리 딜레마 상황을 판단하고 있습니다.\n\n[상황]\n${round.story}\n\n[나의 1차 판단]\n${firstChoice || "A/B 중 선택"}. ${firstChoiceText}\n\n[1차 판단 이유]\n${firstReason || "작성한 선택 이유"}\n\n[우려한 점]\n${concern || "작성한 우려 사항"}\n\n[돌발상황]\n1. ${round.surprises[0]}\n2. ${round.surprises[1]}\n3. ${round.surprises[2]}\n\n[나의 2차 판단]\n${secondChoice || "기존 유지/일부 수정/판단 변경 중 선택"}\n\n[2차 판단 질문 답변]\n${secondReason || "선택별 질문 답변"}\n\n[요청]\n1. 이 판단의 강점과 약점을 구분해 주세요.\n2. 제가 놓친 이해관계자 관점이 있다면 알려주세요.\n3. 기존 유지/일부 수정/판단 변경 중 현재 상황에서 어떤 선택이 더 타당한지 조건부로 비교해 주세요.\n4. 이 선택을 팀원 또는 상사에게 설명할 첫 문장 3개를 제안해 주세요.\n5. 정답처럼 말하지 말고 각 선택의 비용과 실행 조건을 함께 제시해 주세요.\n\n6. 위 분석을 바탕으로, 제가 실제 리더로서 설명할 수 있는 대상별 5줄 설명문을 만들어 주세요.\n\n[대상별 5줄 설명문 작성 조건]\n- 나의 1차 판단과 2차 판단 방향을 유지해 주세요.\n- AI가 새로운 정답을 대신 만들지 말고, 나의 판단을 실무적 리더 언어로 정리해 주세요.\n- 대상별 강조점은 다르게 하되, 핵심 판단 기준은 일관되게 유지해 주세요.\n- 한국 기업의 팀장/리더가 실제 회의, 보고, 1:1에서 사용할 수 있는 표현으로 작성해 주세요.\n- AI 결과는 정답이 아니라 제가 수정해 사용할 수 있는 초안임을 전제로 작성해 주세요.\n\n[작성 대상]\n1. 메인 인물에게 설명할 5줄\n2. 팀원 또는 팀원들에게 설명할 5줄\n3. 상사에게 보고할 5줄\n4. 최종 제출용 5줄 초안\n\n[출력 형식]\n- 각 대상별로 정확히 5줄씩 작성해 주세요.\n- 각 줄은 너무 길지 않게, 실제 말하거나 제출할 수 있는 문장으로 작성해 주세요.\n\n[최종 결정 5줄 입력용 초안]\n아래 5개 문장 형식을 그대로 사용해 작성해 주세요.\n\n[상황 인식 후 작성 절차]\n최종 5줄을 작성하기 전에 반드시 아래 순서로 먼저 판단해 주세요.\n\n1. 원래 상황의 핵심 딜레마가 무엇인지 한 문장으로 정리해 주세요.\n2. 돌발상황 3개가 기존 판단에 어떤 압력이나 위험을 추가했는지 각각 정리해 주세요.\n3. 참여자의 1차 판단과 2차 판단이 어떻게 연결되거나 달라졌는지 정리해 주세요.\n4. 최종 5줄에는 반드시 원래 딜레마의 핵심 갈등과 돌발상황으로 생긴 추가 위험이 반영되어야 합니다.\n5. 일반적인 리더십 문장이 아니라, 이 라운드의 상황·인물·돌발변수에 맞는 문장으로 작성해 주세요.\n\n[최종 결정 5줄 작성 기준]\n1번 문장에는 이 상황의 핵심 딜레마를 반영해 주세요.\n2번 문장에는 참여자의 2차 판단 방향을 분명히 반영해 주세요.\n3번 문장에는 돌발상황 때문에 커진 비용이나 위험을 반영해 주세요.\n4번 문장에는 그 위험을 줄이기 위한 실행 조건을 반영해 주세요.\n5번 문장에는 내일 바로 실행할 수 있는 구체 행동을 작성해 주세요.\n\n1. 나는 이 상황을 ______ 문제로 본다.\n2. 나의 최종 선택은 ______ 이다.\n3. 이 선택의 가장 큰 비용은 ______ 이다.\n4. 그래서 붙일 실행 조건은 ______ 이다.\n5. 내일 바로 할 첫 행동은 ______ 이다.\n\n작성 조건:\n- 위 5줄은 참여자가 앱의 최종 결정 화면에 한 줄씩 그대로 옮겨 적을 수 있게 작성해 주세요.\n- 각 문장은 참여자의 1차 판단, 2차 판단, 선택 이유, 우려한 점, 돌발상황 답변을 반영해 주세요.\n- 나의 판단 방향을 바꾸지 말고, 더 명확하고 실무적인 리더 언어로 정리해 주세요.\n- 추상적인 표현보다 실제 회의에서 말할 수 있는 문장으로 작성해 주세요.\n- 각 줄은 1문장으로 작성해 주세요.\n- 마지막에는 "이 초안은 참여자의 판단을 바탕으로 한 수정 가능한 초안입니다."라고 안내해 주세요.`, [round, firstChoice, firstChoiceText, firstReason, concern, secondChoice, secondReason]);
  const kim = `김박사의 피드백\n\n이번 선택에서 중요한 것은 무엇을 골랐는가보다 그 선택의 비용을 얼마나 알고 있었는가입니다. 당신은 1차로 ${firstChoice || "A/B"}를 선택했고, 돌발상황 이후 ${secondChoice || "2차 판단"}을 선택했습니다.\n\n좋은 판단은 경험에서 나오고, 탁월한 판단은 성찰에서 나옵니다.`;
  const enterPilot = () => { if (!canEnter) return; const cleaned = team.trim(); window.localStorage.setItem(TEAM_STORAGE_KEY, cleaned); setTeam(cleaned); refreshCompleted(cleaned); setScreen(1); };
  const changeTeam = () => { window.localStorage.removeItem(TEAM_STORAGE_KEY); setTeam(""); setCompletedRoundIds([]); reset(); setScreen(0); };
  const startRound = (i: number) => { setRoundIndex(i); loadSavedRound(team.trim(), rounds[i].id); setScreen(3); };
  ...
  return <Shell step={step} scrollRef={scrollRef}><Nav screen={screen} nextDisabled={nextDisabled} onPrev={prev} onHome={home} onNext={navNext} label={navLabel} />
    {screen === 0 && <div className="space-y-6"><div className="rounded-[2rem] bg-gradient-to-br from-blue-50 via-white to-teal-50 p-6"><BrandHeader /><h1 className="mt-4 text-4xl font-black leading-tight text-slate-900">Decision<br />Journey</h1><p className="mt-4 text-sm leading-7 text-slate-500">이번 주 1차수 파일럿 운영용입니다. 팀명만 입력하면 바로 시작할 수 있습니다.</p><ImageHero /></div><Card className="space-y-4 p-5"><TextInput label="팀명" placeholder="예) 1팀 / 새벽등대팀" value={team} onChange={setTeam} />{!canEnter && <RequiredMessage>팀명을 입력해주세요.</RequiredMessage>}<PrimaryButton onClick={enterPilot} disabled={!canEnter}>팀명으로 입장하기</PrimaryButton></Card></div>}
    {screen === 1 && <div className="space-y-5"><div className="rounded-[2rem] bg-gradient-to-br from-blue-50 via-white to-teal-50 p-5"><BrandHeader /><h2 className="mt-4 text-3xl font-black leading-tight text-slate-900">{participantName} 님,<br />오늘의 판단 여정을<br />시작해볼까요?</h2><p className="mt-3 text-sm text-slate-500">저장된 팀명으로 이어서 진행합니다.</p><ImageHero compact /></div><Card className="p-4"><div className="flex items-center justify-between"><div><p className="text-xs font-bold text-slate-400">현재 팀명</p><p className="mt-1 text-lg font-black text-slate-900">{participantName}</p></div><button onClick={changeTeam} className="rounded-2xl bg-slate-100 px-4 py-2 text-xs font-bold text-slate-600">팀명 변경</button></div></Card><Card className="p-5"><div className="grid grid-cols-[120px_1fr] gap-4"><ProgressRing completed={completedCount} /><div><Pill>파일럿 진행 현황</Pill><h3 className="mt-3 text-lg font-black text-slate-900">선택한 라운드 {round.id} · {round.title}</h3><div className="mt-3 flex items-center gap-2">{[1,2,3,4].map((id)=><span key={id} className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${completedRoundIds.includes(id)?"bg-blue-600 text-white":"bg-slate-100 text-slate-400"}`}>{completedRoundIds.includes(id)?"✓":id}</span>)}</div><button onClick={() => startRound(roundIndex)} className="mt-4 w-full rounded-2xl bg-blue-600 px-4 py-3 text-sm font-extrabold text-white">라운드 시작하기 →</button></div></div></Card><div><div className="mb-3 flex items-center justify-between"><b className="text-slate-900">라운드 선택</b><span className="text-sm font-bold text-blue-600">1~4라운드</span></div><div className="grid grid-cols-2 gap-3">{rounds.slice(0,4).map((r,i)=><RoundMiniCard key={r.id} title={r.title} index={i} active={i===roundIndex} done={completedRoundIds.includes(r.id)} onClick={()=>startRound(i)} />)}</div></div><div className="rounded-3xl bg-slate-900 p-5 text-white"><div className="flex items-center gap-4"><div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-white/10 text-3xl">👨‍🏫</div><div><b>김박사의 한마디</b><p className="mt-2 text-sm leading-6 text-slate-200">좋은 판단은 경험에서 나오고, 탁월한 판단은 성찰에서 나옵니다.</p></div></div></div></div>}
    {screen === 2 && <div className="space-y-5"><Pill>라운드 선택</Pill><h2 className="text-2xl font-black text-slate-900">업무관리 딜레마</h2>{rounds.map((r, i) => <button key={r.id} onClick={() => startRound(i)} className="w-full rounded-3xl border border-slate-100 bg-white p-4 text-left shadow-sm"><Pill tone={i % 3 === 0 ? "blue" : i % 3 === 1 ? "teal" : "purple"}>라운드 {r.id}</Pill><h3 className="mt-3 text-lg font-extrabold text-slate-800">{r.title}</h3><p className="mt-1 text-sm text-slate-500">{r.subtitle}</p></button>)}</div>}
    {screen === 3 && <div className="space-y-5"><StepHero badge={`라운드 ${round.id}`} title="상황 카드" subtitle="먼저 현장의 장면과 핵심 질문을 차분히 읽어보세요." icon="🧭" tone="blue"/><Card className="p-5"><div className="mb-3 flex items-center gap-3"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-50 text-2xl">📌</span><h3 className="text-lg font-extrabold text-slate-800">{round.title}</h3></div><p className="text-sm leading-7 text-slate-600">{round.story}</p></Card><Card className="bg-blue-50/70 p-5 text-center"><b className="text-blue-700">핵심 질문</b><p className="mt-2 text-lg font-extrabold leading-8 text-slate-900">{round.question}</p></Card><PrimaryButton onClick={next}>1차 판단하기</PrimaryButton></div>}
    {screen === 4 && <div className="space-y-4"><StepHero badge="1차 판단" title="먼저 직관적으로 판단해보세요" subtitle="정답을 찾기보다 지금 기준에서 무엇을 우선하는지 확인합니다." icon="⚖️" tone="teal"/><FirstOptionCard label="A" title={round.optionA} selected={firstChoice === "A"} onClick={() => setFirstChoice("A")} tone="blue"/><FirstOptionCard label="B" title={round.optionB} selected={firstChoice === "B"} onClick={() => setFirstChoice("B")} tone="teal"/><Card className="space-y-4 p-4"><Pill tone="gray">판단 메모</Pill><TextArea label="선택 이유" placeholder="선택 이유를 적어주세요." value={firstReason} onChange={setFirstReason} /><TextArea label="우려되는 점" placeholder="우려되는 점을 적어주세요." value={concern} onChange={setConcern} /></Card>{!canFirst && <RequiredMessage>A/B 선택, 선택 이유, 우려되는 점을 모두 입력해주세요.</RequiredMessage>}<PrimaryButton onClick={next} disabled={!canFirst}>돌발상황 확인하기</PrimaryButton></div>}
    ...
    {screen === 10 && <div className="space-y-5"><StepHero badge="AI 모범 프롬프트" title="프롬프트를 복사해 AI 도구에 붙여넣으세요" subtitle="내 판단과 선택별 답변이 자동으로 반영됩니다." icon="🤖" tone="purple"/><Card className="space-y-4 p-4"><pre className="max-h-72 overflow-y-auto whitespace-pre-wrap rounded-2xl bg-slate-50 p-4 text-xs leading-6 text-slate-700">{prompt}</pre><button onClick={async () => { await navigator.clipboard.writeText(prompt); setCopied(true); }} className="w-full rounded-2xl bg-violet-600 px-4 py-3 text-sm font-extrabold text-white">{copied ? "복사 완료" : "프롬프트 복사하기"}</button></Card>
      ...
      <TextArea label="AI 답변 전체 붙여넣기" placeholder="Gemini 또는 ChatGPT가 생성한 답변 전체를 붙여넣어 주세요. 최종 결정 5줄 초안을 불러오는 데 사용됩니다." value={aiSummary} onChange={setAiSummary} rows={6} />{!canAi && <RequiredMessage>AI 답변 전체를 붙여넣어 주세요.</RequiredMessage>}<PrimaryButton onClick={next} disabled={!canAi}>AI답변 검토하기</PrimaryButton>
